...
```bash
mkdir -p "$TARGET"/{app/api/generate,app/api/agent-card,app/api/a2a,content,data,scripts}
```
...
### Step 3 — Write the 17 boilerplate files
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      ...
      // agents (and a mesh like MeshedIn) can discover and register it by
      // reading the card.
...
The agent card + A2A endpoint are provided by the livecv package (≥1.3.0) — keep
these route files thin wrappers. `buildAgentCard(config, { baseUrl })` returns the
lean A2A card (spec 0.3.0); it includes `lookingFor` + `offering` so a mesh can
match on them. Specifics are answered LIVE at `/api/a2a` from the KB, not frozen here.

```ts
import { buildAgentCard } from "livecv";
import config from "@/livecv.config";

// Served at /.well-known/agent-card.json via the next.config rewrite.
export const dynamic = "force-dynamic";

export async function GET(req: Request) {
  const origin = new URL(req.url).origin;
  return Response.json(buildAgentCard(config, { baseUrl: origin }), {
...
// A2A JSON-RPC `message/send` — another agent asks; this answers LIVE from the
// persona's KB. The handler is in the livecv package; just wire it up.
...
Register-on-deploy: publishes this persona's card into a [MeshedIn](https://github.com/sayantan94/meshedin) registry so its agent becomes discoverable + interviewable.

```js
// node scripts/register.mjs [siteUrl] [meshUrl]
// Defaults: local dev site on :3000, MeshedIn on :3737.
const site = process.argv[2] ?? process.env.SITE_URL ?? "http://localhost:3000";
const mesh = process.argv[3] ?? process.env.MESH_URL ?? "http://localhost:3737";

const res = await fetch(`${site}/api/agent-card`);
if (!res.ok) {
  console.error(`could not fetch agent card from ${site}/api/agent-card: ${res.status}`);
  process.exit(1);
}
const card = await res.json();

const reg = await fetch(`${mesh}/api/register`, {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({ card }),
});
const out = await reg.json().catch(() => ({}));
if (reg.ok) console.log(`✓ registered ${card.name} into the mesh at ${mesh}`);
else {
  console.error(`✗ register failed (${reg.status}): ${JSON.stringify(out)}`);
  process.exit(1);
}
```
...
export const identity = defineIdentity({
  ...
  principles,
  lookingFor: [
    // TODO: What you're open to — roles, consulting, collaborations.
    // The mesh matches on this — be specific.
  ],
  // What you bring to a match (the mesh uses this for mutual fit).
  offering: [
    // TODO: 2-4 things you bring — experience, skills, what you're great at.
  ],
});

// ── Mesh opt-in ──────────────────────────────────────────────────────
// Set to true to allow this persona to join a MeshedIn network. Consent-gated:
// only your own persona, only when you choose. Register with: npm run register
export const consented = false;
```
...
```
✓ Scaffold complete — 20 files in {TARGET}
...
4. Write the 17 boilerplate files with `{{...}}` substituted.
5. Generate `data/{identity,career,projects}.ts` from extracted data.
6. Print next-steps panel.

Result: 20 files in `./my-site/`. `cd my-site && npm install && npm run dev` produces a working site that also publishes its A2A agent card.
...
1. Validate target.
2. Skip extraction.
3. Write all 20 files with placeholder substitutions (`"Your Name"`, `"TODO: ..."`).
...
- **All 20 files required.** The project won't compile if any are missing.
